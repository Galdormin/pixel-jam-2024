...
func _on_timer_timeout():
	...
	var floor = create_floor_card()
	floor.z_index = 0
	floor.update_position(slot.get_card_position(floor.global_position))
	
	var worm = create_worm_card()
	worm.z_index = 0
	slot.add_card(worm, false)
...
func create_floor_card() -> Card:
	var card = database.create_card_by_name("Floor")
	card.z_index = 1
...
func create_worm_card() -> Card:
	var card = database.create_card_by_name("Worm")
	card.z_index = 1
